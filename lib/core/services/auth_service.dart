  ...
  Future<void> _migrateHiveData({required String fromId, required String toId}) async {
    try {
      ...
      if (Hive.isBoxOpen(oldBoxName) || await Hive.boxExists(oldBoxName)) {
        // dynamic 타입으로 열어야 Map<String, dynamic> 캐스팅 오류 방지
        final oldBox = await Hive.openBox<dynamic>(oldBoxName);
        final newBox = await Hive.openBox<dynamic>('${boxName}_$toId');
      ...
      // 마이그레이션 실패해도 앱은 계속 실행 — 데이터 손실 없음
    }
  }
}
